...
void Lora_RX_Sensor() {
  ...
  if ( str.indexOf("ok") == 0 )
  {
    ...
    if ( str.indexOf("radio_rx") == 0 )  //checking if data was reeived (equals radio_rx = <data>). indexOf returns position of "radio_rx"
    {
      ...
      sensor_sleep_counter+=1;

      if(sensor_sleep_counter==5) {
        loraSerial.print("radio tx ");
        loraSerial.println("99999909");
...
void Lora_TX_Actuator(bool actuator_value) {

  Serial.print("packageID = ");
  Serial.println(packageID);
  loraSerial.print("radio tx ");
  loraSerial.println(actuator_value);
  //loraSerial.println(packageID);
  str = loraSerial.readStringUntil('\n');
  Serial.println(str);
  str = loraSerial.readStringUntil('\n');
  ...
  if ( str.indexOf("ok") == 0 )
  {
    ...
    {
      Serial.println("Radio rx");
      actuator_sleep_counter+=1;
      
      if(actuator_sleep_counter==10) {
